function init3dhop() {
  if (isIOS()) $('head').append('<meta name="viewport" content="width=device-width">');
  ...
  $('#3dhop').on('touchend pointerup touchmove', function(e) {clearInterval(interval);})
...
function hotspotSwitch(on) {
  ...
  swapImage = $('#hotspot').attr('data-swap')
  currentImage = $('#hotspot').attr('src');
  $('#hotspot').attr({'src': swapImage, 'data-swap': currentImage });
}
